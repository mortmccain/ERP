...
public sealed class Sale : AggregateRoot
{
    ...
    /// <summary>
    /// Subtotal after applying all line-level discounts and FOC adjustments.
    /// This is the amount used as the base for sale-level discount calculations.
    /// </summary>
    public Money SubTotalAfterLineDiscounts { get; private set; }
...
#pragma warning restore CS8618
...

    {
        ...
        if (createdByUserId == Guid.Empty) throw new DomainException("Seller ID (Created by ID) is required ");
        if (string.IsNullOrWhiteSpace(createdByName)) throw new DomainException("Name of the seller (created by name) is required");
